...
var EXTREME_SCALING_FACTOR = 0.4;
...
std_diff.prototype.calc = function(params) {
  ...
  var stats = new std_beatmap_stats({cs: map.cs}).with_mods(mods);
  ...
  this.speed_length_bonus = this._length_bonus(this.speed,
  ...
  this.total = (
    this.aim + this.speed + Math.abs(this.speed - this.aim) *
...
std_diff.prototype._calc_strain = function(type, diffobj, prev_diffobj,
  speed_mul)
{
  var obj = diffobj.obj;
...
std_ppv2.prototype.calc = function(params) {
  ...
  // if (nobjects < 2000) {
  //   length_bonus -= 0.5*(2000 - nobjects) / 2000;
  // }
  ...
  var acc = (
    Math.pow(1.4, mapstats.od) * Math.pow(Math.max(1, mapstats.ar/10), 3)*
    Math.pow(real_acc, 12.0) * 10
  );
    //Drastically change acc calculation to fit current droid meta
  ...
  var final_multiplier = 1.15;
  //Slight buff to final value
  if (mods & modbits.nf) final_multiplier *= 0.90;
  if (mods & modbits.so) final_multiplier *= 0.95;

  var extreme_penalty = Math.pow((1-Math.abs(speed - Math.pow(aim,1.1))/Math.max(speed, Math.pow(aim,1.1))), 0.2);
  extreme_penalty = Math.max(Math.pow(extreme_penalty , 2), (-2 * (Math.pow(1 - extreme_penalty, 2)) + 1));
    //Extreme_penalty : added to penaltize map with little aim but ridiculously high speed value (which is easily abusable by using more than 2 fingers)

  this.total = Math.pow(
    Math.pow(aim, 1.1) + Math.pow(speed, 1.1) +
    Math.pow(acc, 1.1),
    1.0 / 1.1
  ) * final_multiplier * extreme_penalty;
  // Add Extreme_penalty to final calculation as factor
